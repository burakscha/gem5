...
def parse_spill_stats(spill_file):
        ...
        for line_num, line in enumerate(f):
            ...
            if len(parts) >= 9:
                try:
                    ...
                    spill_stats['min_tick_diff'] = min(spill_stats['min_tick_diff'], tick_diff)
                    spill_stats['max_tick_diff'] = max(spill_stats['max_tick_diff'], tick_diff)
    ...
    if spill_stats['total_spills'] > 0:
        spill_stats['avg_tick_diff'] = total_tick_diff / spill_stats['total_spills']
    ...
    spill_stats['unique_memory_count'] = len(spill_stats['unique_memory_addresses'])
...
def print_analysis(stats, spill_stats, m5out_dir):
    ...
    print("-" * 80)
    if stats:
        print(f"  Total Instructions:        {stats.get('total_instructions', 'N/A'):>15,}")
        print(f"  Total Operations:          {stats.get('total_operations', 'N/A'):>15,}")
        print(f"  Total Load Instructions:   {stats.get('total_loads', 'N/A'):>15,}")
        print(f"  Total Store Instructions:  {stats.get('total_stores', 'N/A'):>15,}")
        print(f"  CPU Cycles:                {stats.get('cpu_cycles', 'N/A'):>15,}")
        print(f"  Simulation Ticks:          {stats.get('simulation_ticks', 'N/A'):>15,}")
        print(f"  Simulation Time:           {stats.get('simulation_seconds', 'N/A'):>15.6f} seconds")
        print(f"  CPI (Cycles per Inst):     {stats.get('cpi', 'N/A'):>15.4f}")
        print(f"  IPC (Inst per Cycle):      {stats.get('ipc', 'N/A'):>15.4f}")
        if 'dcache_accesses' in stats:
            print(f"  DCache Accesses:           {stats.get('dcache_accesses', 'N/A'):>15,}")
        if 'dcache_misses' in stats:
            print(f"  DCache Misses:             {stats.get('dcache_misses', 'N/A'):>15,}")
    else:
    ...
    if total_insts > 0:
        ...
        print(f"  Load Instructions:         {total_loads:>15,}  ({load_pct:>6.2f}%)")
        print(f"  Store Instructions:        {total_stores:>15,}  ({store_pct:>6.2f}%)")
        print(f"  Spill Events:              {total_spills:>15,}  ({spill_pct:>6.2f}%)")
        print()
    ...
    print("-" * 80)
    if spill_stats['total_spills'] > 0:
        print(f"  Total Spills Detected:     {spill_stats['total_spills']:>15,}")
        print(f"  Unique Store PCs:          {spill_stats['unique_store_count']:>15,}")
        print(f"  Unique Load PCs:           {spill_stats['unique_load_count']:>15,}")
        print(f"  Unique Memory Addresses:   {spill_stats['unique_memory_count']:>15,}")
        print()
        print(f"  Average Tick Difference:   {spill_stats['avg_tick_diff']:>15,.2f}")
        print(f"  Min Tick Difference:       {spill_stats['min_tick_diff']:>15,}")
        print(f"  Max Tick Difference:       {spill_stats['max_tick_diff']:>15,}")
        print()
        
        # Top 5 hottest store locations
        print("  🔥 Top 5 Hottest Store PCs:")
        top_stores = sorted(spill_stats['store_counts'].items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        top_loads = sorted(spill_stats['load_counts'].items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        top_addrs = sorted(spill_stats['memory_address_counts'].items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        if spill_stats['sample_spills']:
            ...
            for i, spill in enumerate(spill_stats['sample_spills'][:5], 1):
                print(f"     {i}. Store PC: {spill['store_pc']}, Load PC: {spill['load_pc']}, "
...
def save_txt_report(stats, spill_stats, m5out_dir):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("-" * 80 + "\n")
        if stats:
            f.write(f"  Total Instructions:        {stats.get('total_instructions', 'N/A'):>15,}\n")
            f.write(f"  Total Operations:          {stats.get('total_operations', 'N/A'):>15,}\n")
            f.write(f"  Total Load Instructions:   {stats.get('total_loads', 'N/A'):>15,}\n")
            f.write(f"  Total Store Instructions:  {stats.get('total_stores', 'N/A'):>15,}\n")
            f.write(f"  CPU Cycles:                {stats.get('cpu_cycles', 'N/A'):>15,}\n")
            f.write(f"  Simulation Ticks:          {stats.get('simulation_ticks', 'N/A'):>15,}\n")
            f.write(f"  Simulation Time:           {stats.get('simulation_seconds', 'N/A'):>15.6f} seconds\n")
            f.write(f"  CPI (Cycles per Inst):     {stats.get('cpi', 'N/A'):>15.4f}\n")
            f.write(f"  IPC (Inst per Cycle):      {stats.get('ipc', 'N/A'):>15.4f}\n")
            if 'dcache_accesses' in stats:
                f.write(f"  DCache Accesses:           {stats.get('dcache_accesses', 'N/A'):>15,}\n")
            if 'dcache_misses' in stats:
                f.write(f"  DCache Misses:             {stats.get('dcache_misses', 'N/A'):>15,}\n")
        else:
        ...
        if total_insts > 0:
            ...
            f.write(f"  Load Instructions:         {total_loads:>15,}  ({load_pct:>6.2f}%)\n")
            f.write(f"  Store Instructions:        {total_stores:>15,}  ({store_pct:>6.2f}%)\n")
            f.write(f"  Spill Events:              {total_spills:>15,}  ({spill_pct:>6.2f}%)\n")
            f.write("\n")
            
            # Additional ratios
            if total_loads > 0:
                spill_to_load = (total_spills / total_loads) * 100
                f.write(f"  Spills as % of Loads:      {spill_to_load:>22.2f}%\n")
            if total_stores > 0:
                spill_to_store = (total_spills / total_stores) * 100
                f.write(f"  Spills as % of Stores:     {spill_to_store:>22.2f}%\n")
        else:
        ...
        f.write("-" * 80 + "\n")
        if spill_stats['total_spills'] > 0:
            f.write(f"  Total Spills Detected:     {spill_stats['total_spills']:>15,}\n")
            f.write(f"  Unique Store PCs:          {spill_stats['unique_store_count']:>15,}\n")
            f.write(f"  Unique Load PCs:           {spill_stats['unique_load_count']:>15,}\n")
            f.write(f"  Unique Memory Addresses:   {spill_stats['unique_memory_count']:>15,}\n")
            f.write("\n")
            f.write(f"  Average Tick Difference:   {spill_stats['avg_tick_diff']:>15,.2f}\n")
            f.write(f"  Min Tick Difference:       {spill_stats['min_tick_diff']:>15,}\n")
            f.write(f"  Max Tick Difference:       {spill_stats['max_tick_diff']:>15,}\n")
            f.write("\n")
            ...
            if spill_stats['sample_spills']:
                ...
                for i, spill in enumerate(spill_stats['sample_spills'][:5], 1):
                    f.write(f"     {i}. Store PC: {spill['store_pc']}, Load PC: {spill['load_pc']}, "
                          f"Mem: {spill['memory_addr']}, ΔTicks: {spill['tick_diff']:,}\n")
        else:
        ...
        if roi_active:
            f.write(f"  Instructions in ROI: {stats.get('total_instructions', 0):,}\n")
        f.write("\n")
