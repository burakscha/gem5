#!/bin/bash
# run.sh — simulate the l1_miss spill-store micro-test
#
# Cache config (same as load tests for consistency):
#   L1 dcache : 1 kB, 4-way, 64 B  → 16 total lines (4 sets × 4 ways)
#   L2        : 8 kB, 4-way, 64 B  → 128 total lines
#   Memory    : SimpleMemory (30 ns flat latency)
#
# The 32-line BSS flush (before ROI) evicts main's stack frame line
# from L1.  The store inside the ROI then triggers an RFO miss.
#
# Usage:
#   cd <this directory>
...
echo "=== store/l1_miss: running gem5 ==="
"$GEM5" \
    --outdir="$OUTDIR" \
    "$CFG" \
    --cmd="$BIN" \
