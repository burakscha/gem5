#!/bin/bash
# run.sh — simulate the l1miss_l2miss spill-store micro-test
#
# Cache config:
#   L1 dcache : 1 kB, 4-way, 64 B  → 4 sets × 4 lines  = 16 total lines
#   L2        : 8 kB, 4-way, 64 B  → 32 sets × 4 lines  = 128 total lines
#   Memory    : SimpleMemory (30 ns flat latency)
#
# 160-line BSS flush evicts x's stack line from BOTH L1 and L2:
#   160 / 4  L1 sets = 40 lines per set >> 4 ways → L1 eviction ✓
#   160 / 32 L2 sets = 5  lines per set >  4 ways → L2 eviction ✓
#
# Usage:
#   cd <this directory>
...
echo "=== store/l1miss_l2miss: running gem5 ==="
"$GEM5" \
    --outdir="$OUTDIR" \
    "$CFG" \
    --cmd="$BIN" \
